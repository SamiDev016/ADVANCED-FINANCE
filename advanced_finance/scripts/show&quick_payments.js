        ...
        primary_action: function(data) {
            frappe.call({
                ...
                error: function(r) {
                    ...
                    let error_message = __('An unknown error occurred while submitting payment.');
                    
                    if (r.exc) {
                        const traceback_lines = r.exc.split('\n');
                        if (traceback_lines.length > 0) {
                            const first_line = traceback_lines[0];
                            const match = first_line.match(/(ValidationError|Exception|DataError|DuplicateEntryError): (.*)/);
                            
                            if (match && match[2]) {
                                error_message = match[2].trim().replace(/^\["|"]$/g, '');
                            }
                        }
                    }
